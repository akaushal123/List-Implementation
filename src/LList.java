 ...
 * 
 * 
 * Modifications were made by Amit K Dogra: The toString method is overwritten
 * to give a nice display of the items in the list in this format { <1> <2> <3>
 * <4> }
 * 
 * An alternate display method has been created to print the list one item to a
 * line along with the index
 * 
...
class LList<T> implements ListInterface<T> {
	...
	public void reverse() {
		...
		int low = 1, high = numberOfEntries;
		while (low <= high) {
			T temp = this.getEntry(low);
			this.replace(low, this.getEntry(high));
			this.replace(high, temp);
			low++;
			high--;
		}// end while
	}//end reverse
	...
	public void cycle() {
		...
		if (numberOfEntries > 1) {
			T first = this.getEntry(1);
			for (int index = 1; index < numberOfEntries; index++) {
				this.replace(index, this.getEntry(index + 1));
			}
			this.replace(numberOfEntries, first);
		}
	}
}
